...
export const uploadCategoriaImage = multer({ storage: storage }).single(
  "imagen"
);
...
export const obtenerSubcategoriasPorCategoria = (req, res) => {
  ...
  if (!categoria_id) {
    return res
      .status(400)
      .json({ message: "El parámetro 'categoria_id' es requerido." });
  }
  try {
    const subcategorias =
      categoriasModel.subcategoriasPorCategoria(categoria_id);
    res.json(subcategorias);
  } catch (error) {
    console.error(
      `Error al obtener subcategorías para la categoría ${categoria_id}:`,
      error
    );
...
export const actualizarDestacadas = (req, res) => {
  ...
  if (!Array.isArray(ids) || ids.length > 4) {
    return res
      .status(400)
      .json({ message: "Se requiere un array con un máximo de 4 IDs." });
  }
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Categorías destacadas actualizadas correctamente." });
  } catch (error) {
...
export const crearNuevaCategoria = (req, res) => {
  ...
  if (
    ...
    return res
      .status(400)
      .json({ message: "Debe proporcionar al menos una subcategoría." });
  }
  nombre = nombre.charAt(0).toUpperCase() + nombre.slice(1);

  // Verificar si ya existe una categoría con el mismo nombre
  const categoriaExistente = categoriasModel.obtenerCategoriaPorNombre(
    nombre.trim()
  );
  if (categoriaExistente) {
    return res
      .status(409)
      .json({ message: "Ya existe una categoría con ese nombre." }); // 409 Conflict
  }

  try {
    const nuevoId = categoriasModel.crearCategoriaConSubcategorias({
      nombre: nombre.trim(),
      subcategorias,
      color,
      imagen_url,
    ...
    res
      .status(201)
      .json({ message: "Categoría creada exitosamente", id: nuevoId });
  } catch (error) {
    ...
    res
      ...
      .json({
        message:
          error.message || "Error interno del servidor al crear la categoría.",
      });
  }
};
...
export const actualizarCategoria = (req, res) => {
  ...
  try {
    ...
    const categoriaExistente = categoriasModel
      .obtenerCategorias()
      .find((c) => c.id == id);
    if (!categoriaExistente) {
    ...
    const imagenAnterior = categoriaExistente.imagen_url;

    // Si se sube un nuevo archivo, se usa. Si no, se mantiene el anterior.
    const imagen_url = req.file
      ? `/uploads/categoryImages/${req.file.filename}`
      : categoriaExistente.imagen_url;

    categoriasModel.actualizarCategoriaConSubcategorias(id, {
      nombre: nombre.trim(),
      subcategorias,
      color,
      imagen_url,
    ...
    // Si se subió una nueva imagen, eliminar la anterior (si no es la de por defecto)
    if (req.file && imagenAnterior && !imagenAnterior.includes("default.png")) {
      const imagePath = path.join(__dirname, "../../public", imagenAnterior);
      fs.unlink(imagePath, (err) => {
        if (err) {
          // Loguear el error pero no impedir la respuesta exitosa, ya que la categoría sí se actualizó.
          console.error("Error al eliminar la imagen anterior:", err);
        }
      });
    }

    // Enviar siempre una respuesta exitosa después de la actualización.
    res.status(200).json({ message: "Categoría actualizada exitosamente", id });
  } catch (error) {
    console.error(
      `Error en el controlador al actualizar categoría ${id}:`,
      error
    );
...
export const eliminarCategoria = (req, res) => {
  ...
  try {
    ...
    if (success) {
      ...
      res
        .status(404)
        .json({ message: "Categoría no encontrada o no se pudo eliminar." });
    }
  } catch (error) {
    ...
    if (error.code === "SQLITE_CONSTRAINT_FOREIGNKEY") {
      return res
        ...
        .json({
          message:
            "No se puede eliminar. La categoría está en uso por uno o más cuadros.",
        });
    }
    res.status(500).json({ message: "Error interno del servidor." });
  }
