...
export async function CategoriasCRUD(notyf, modals) {
  ...
  /**
   * displayError - Muestra un mensaje de error en el elemento especificado.
   * @param {HTMLElement} element - El elemento donde mostrar el error.
  ...
  function clearForm() {
    ...
    $$(".color-option.ring-2").forEach((el) =>
  ...
  function addSubcategoria() {
    ...
    if (!val) {
      ...
      displayError(subcatError, "La subcategoría no puede estar vacía.");
      return;
    }
  ...
  subcatInput.addEventListener("focus", () => {
    displayError(
      subcatError,
      "Presiona Enter o 'Agregar' para añadir a la lista."
    );
  ...
  function validateForm() {
    ...
    if (subcategorias.length === 0) {
      displayError(subcatError, "Debes añadir al menos una subcategoría.");
      subcatInput.focus();
      isValid = false;
    }
    return isValid;
  ...
  const setupEditModal = (data) => {
    ...
    if (imagenPreview)
      imagenPreview.src = data.imagen_url || "/uploads/default.png";
  ...
  const handleEditClick = (id, targetButton) => {
    ...
    const data = {
      ...
      subcategorias:
        row.cells[4].textContent.trim() === "Sin subcategorías"
          ? []
  ...
  const handleDeleteClick = async (id, targetButton) => {
    ...
    try {
      await modals.confirmarAccion(
        `¿Estás seguro de que quieres eliminar la categoría "${nombreCategoria}"? Esto también eliminará sus subcategorías.`
      );
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      clearForm();
      await reloadTableData(); // <-- Esta línea es clave para la actualización instantánea
    } catch (error) {
      console.error(
  ...
  function renderCategoriasTable(categoriasData) {
    ...
    if (categoriasData.length === 0) {
      tablaCategoriasBody.innerHTML =
        '<tr><td colspan="5" class="text-center p-4 text-zinc-500">No hay categorías para mostrar.</td></tr>';
      return;
    }
    categoriasData.forEach((cat) => {
      ...
      const accionesHtml =
        ...
        (window.currentUser.rol === "admin" ||
          ...
          : "";
      const imagenHtml = `<img src="${
        cat.imagen_url || "/uploads/default.png"
      }" class="w-10 h-10 object-cover rounded-md" alt="Imagen de ${escapeHtml(
        cat.nombre
      )}">`;
      const colorIndicator = cat.color
        ? `<div class="w-4 h-4 rounded-full" style="background-color: ${escapeHtml(
            cat.color
          )};"></div>`
        : "";
      tr.innerHTML = `
                ...
                <td class="px-4 py-3 text-sm font-medium">${escapeHtml(
                  cat.nombre
                )}</td>
                <td class="px-4 py-3 text-sm text-zinc-400">${escapeHtml(
                  cat.subcategorias || "Sin subcategorías"
                )}</td>
  ...
  function renderColorPicker() {
    ...
    coloresPredefinidos.forEach((color) => {
      ...
      colorEl.className =
        "color-option w-8 h-8 rounded-full cursor-pointer transition-transform hover:scale-110";
